...
public class EstadoTestSerialize {

    @Test
    public void SerializarEstadoYDeserializar() throws IOException, ClassNotFoundException {
        ...
        estado.serializar(bytes);
        // deserializar
        var objetoDeserializado = Estado.deserializar(new ByteArrayInputStream(bytes.toByteArray()));

        assertNotNull(objetoDeserializado);
        // [...] verificar que `objeto` y `objetoDeserializado` son iguales
        verificarMismasDescripciones(estado.generarDescripcion(), objetoDeserializado.generarDescripcion());
    }
